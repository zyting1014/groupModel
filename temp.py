...
# 将日期变量转为int
def parseDateToInt_old(data0, columns):
    def change(x):
        if isinstance(x, str):
            if x.find('/') != -1:
...
# 将日期变量由int转为Datetime
def int_to_datetime(data0, columns):
    data = data0.copy()
    cnt = data.shape[0]
    for column in columns:
        print('正在转化%s列..' % column)
        temp = pd.to_datetime(data[column], format='%Y%m%d', errors='coerce')
        #         if temp.isnull().sum() == cnt:
        #             temp = pd.to_datetime(data[column],format='%Y-%m-%d %H:%M:%S', errors='coerce')
        data[column] = temp
        #         print(data[column].head())
        assert data[column].isnull().sum() != cnt, str(column) + '列转化失败！请检查..'
    return data

# 删除以ctime或mis_date开头的列
def ctime_and_mis_date_date(data):
    drop_list = []
    columns = data.columns
    for column in columns:
        if column.startswith('ctime') or column.startswith('mis_date'):
...
def supervised_method(df_train, df_test):
    import GroupFunc
    # 决策树top10
    # df_train, column_name = GroupFunc.decisionTreeMethod4(df_train, "creditlimitamount_4", 65954.0, "opendate_4",
    #                                                       20150523.0, "creditlimitamount_4", 329720.0, "var_jb_23",
    #                                                       10.5, "nasrdw_recd_date", 20181023.0, "opendate_3",
    #                                                       20161105.0, "opendate_3", 20160903.0, False)
    # df_test, column_name = GroupFunc.decisionTreeMethod4(df_test, "creditlimitamount_4", 65954.0, "opendate_4",
    #                                                      20150523.0, "creditlimitamount_4", 329720.0, "var_jb_23", 10.5,
    #                                                      "nasrdw_recd_date", 20181023.0, "opendate_3", 20161105.0,
    #                                                      "opendate_3", 20160903.0)

    # Xgboost top10
    # df_train, column_name = GroupFunc.decisionTreeMethod4(df_train, "creditlimitamount_4", 32188.5, "nasrdw_recd_date",
    #                                                       20181024, "opendate_3", 20170616, "opendate_4", 20141226,
    #                                                       "var_jb_94", 6, "creditlimitamount_4", 248250, "var_jb_94",
    #                                                       571.5, False)
    # df_test, column_name = GroupFunc.decisionTreeMethod4(df_test, "creditlimitamount_4", 32188.5, "nasrdw_recd_date",
    #                                                      20181024, "opendate_3", 20170616, "opendate_4", 20141226,
    #                                                      "var_jb_94", 6, "creditlimitamount_4", 248250, "var_jb_94",
    #                                                      571.5)

    # df_train, column_name = GroupFunc.decisionTreeMethod4(df_train, "creditlimitamount_4", 65954.0,
    #                                                       "creditlimitamount_4", 299.5, "creditlimitamount_4", 329720.0,
    #                                                       "var_jb_22", 10.5, "nasrdw_recd_date", 20181023.0,
    #                                                       "var_jb_22", 64.5, "creditlimitamount_4", 509500.0)
    # df_test, column_name = GroupFunc.decisionTreeMethod4(df_test, "creditlimitamount_4", 65954.0, "creditlimitamount_4",
    #                                                      299.5, "creditlimitamount_4", 329720.0, "var_jb_22", 10.5,
    #                                                      "nasrdw_recd_date", 20181023.0, "var_jb_22", 64.5,
    #                                                      "creditlimitamount_4", 509500.0)

    # df_train, column_name = GroupFunc.decisionTreeMethod4(df_train, "creditlimitamount_4", 32188.5, "nasrdw_recd_date",
    #                                                       20181024, "var_jb_22", 13.5, "var_jb_23", 39.5, "var_jb_94",
    #                                                       6, "creditlimitamount_4", 80855, "creditlimitamount_4",
    #                                                       245020)
    # df_test, column_name = GroupFunc.decisionTreeMethod4(df_test, "creditlimitamount_4", 32188.5, "nasrdw_recd_date",
    #                                                      20181024, "var_jb_22", 13.5, "var_jb_23", 39.5, "var_jb_94", 6,
    #                                                      "creditlimitamount_4", 80855, "creditlimitamount_4", 245020)
...
def main():
    ...
    ##################################################
    # 删除查询时间字段
    drop_list = ctime_and_mis_date_date(df_train)
    df_train = df_train.drop(drop_list, axis=1)
    df_test = df_test.drop(drop_list, axis=1)

    # 日期特征处理
    feature_date = getFeatureDate(df_train)
    df_train = parseDateToInt_old(df_train, feature_date)
    df_test = parseDateToInt_old(df_test, feature_date)
    # 类别特征处理
    df_train, df_test = CategoryPCA(df_train, df_test, getFeatureCategorical(df_train))
    ##################################################
